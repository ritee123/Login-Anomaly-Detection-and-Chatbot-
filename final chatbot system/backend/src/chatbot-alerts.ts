...
let lastAlertTimestamps: Record<string, string> = {};
  ...
  try {
    ...
    for (const a of alerts) {
      ...
      if (!lastAlertTimestamps[a.email] || lastAlertTimestamps[a.email] !== a.timestamp) {
        ...
        console.log(
          `REAL-TIME ALERT: Suspicious login attempt detected: 5 failed logins for ${a.email}\nRisk Level: ${a.risk_level}\nAction: Suggest Black IP notify-admin`
        );
...
app.get('/chatbot', async (req: any, res: any) => {
  ...
  if (typeof question === 'string' && question.toLowerCase().includes('suspicious login')) {
    ...
    try {
      ...
      const messages = alerts.map((a: any) =>
        `Suspicious login attempt detected: 5 failed logins for ${a.email}\nRisk Level: ${a.risk_level}\nAction: Suggest Black IP notify-admin`
      );
